// Main-visual videos (MainVisual.astro): a muted loop layered over the
// project's main image, which stays the thing every morph measures and
// flies. It's shown only once it has a frame, so the image stands in while
// it loads.
//
// When it plays: once "warm" (warmUpVideos - started while the page/view
...
// the video resumed). Not yet warm, it plays only while on screen.
//
// Morphs: the playing video itself flies (liftVideo -> landVideo). It's
// moved into a fixed "flight host" (#morph-video-host, Layout.astro) that
// is animated with the morph clone, then dropped into the destination's
// slot in place of that video. A media element moved within one task keeps
// playing, so the picture never stops - across a page swap too, since the
// host is transition:persist.

const SELECTOR = 'video[data-main-video]';

let observer: IntersectionObserver | null = null;
// Fallback for a video reached before warmUpVideos got to it: starts its
// download about one screen ahead, so its reveal doesn't wait on arrival.
...
// Shared with project-page.ts's content videos (both are preload="none").
...
// The image the video sits on (ProjectImage: <img> or <picture><img>).
...
// visibility:hidden until it has played a frame, so checking it would mean
// it never plays. (Not the parent either - .morph-link is display:contents,
// which checkVisibility always reports as not visible.) The image is hidden
...
export function refreshMainVideos(): void {
  document.querySelectorAll<HTMLVideoElement>(SELECTOR).forEach((video) => {
    ...
    const shown = isShown(video) && !reducedMotion();
...
// Starts (and keeps playing, see refreshMainVideos) every video the
// page/view can show while nothing moves: once it has settled, one per idle
// period - so scrolling never has to start one. The approach preloader
// above stays as a fallback for a video reached before its turn.
...
export function warmUpVideos(): void {
  ...
  warmUpTimer = window.setTimeout(() => {
    const pending = Array.from(document.querySelectorAll<HTMLVideoElement>(SELECTOR));
    const next = () => {
      const video = pending.shift();
      if (!video) return;
      if (!video.dataset.flying && isShown(video) && !reducedMotion()) {
...
export function initMainVideos(root: ParentNode = document): { destroy: () => void } | undefined {
  const videos = Array.from(root.querySelectorAll<HTMLVideoElement>(SELECTOR)).filter(
  ...
  preloader = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        const video = entry.target as HTMLVideoElement;
        if (!entry.isIntersecting || !isShown(video) || reducedMotion()) return;
...
// The video layered over a main image, if any. ProjectImage renders either
// <img> or <picture><img>, with the video right after it.
function videoFor(img: Element | null | undefined): HTMLVideoElement | null {
  ...
  return next instanceof HTMLVideoElement && next.matches(SELECTOR) ? next : null;
}
...
export function whenVideoReady(img: Element | null | undefined): Promise<void> {
  ...
  if (!video || reducedMotion()) return Promise.resolve();
  startLoading(video);
